...
global func FxDiscoTimer(object pTarget, int iNumber)
{
...
public func RelaunchPlayer(iPlr, object iClonk, int iRelaunchs, iKiller, int iKills)
{
	...
	if (iKiller != NO_OWNER && iKiller != iPlr && GetPlayerTeam(iKiller) != GetPlayerTeam(iPlr))
		if (GetCrew(iKiller))
		{
			CreateContents(COKI, GetCrew(iKiller));
	...
	if (iKiller == NO_OWNER || iKiller == iPlr)
		...
		{
			Log("$NoRelaunch$", GetTaggedPlayerName(iPlr));
			InitScoreboard(iPlr, 1, iKills);
			return 0;
		}
	if (iKiller != NO_OWNER && iKiller != iPlr)
		if (LocalN("Relaunchs", iClonk) == 2)
		{
			Log("$NoRelaunchWithKiller$", GetTaggedPlayerName(iPlr), GetTaggedPlayerName(iKiller));
			InitScoreboard(iPlr, 1, iKills);
			return 0;
		}
	...
	if (iKiller == NO_OWNER || iKiller == iPlr)
		Log("$Relaunch$", GetTaggedPlayerName(iPlr), 2 - LocalN("Relaunchs", clnk));
	if (iKiller != NO_OWNER && iKiller != iPlr)
		Log("$RelaunchWithKiller$", GetTaggedPlayerName(iPlr), 2 - LocalN("Relaunchs", clnk), GetTaggedPlayerName(iKiller));
...
global func NewWagon()
{
	...
	var wag;
	if (!dir)
		wag = CreateObject(WAGN, LandscapeWidth() - 20, Position(), -1);
	if (dir)
		wag = CreateObject(WAGN, 20, Position(), -1);
	if (wag)
		wag->Set(dir);
	
	if (!Random(15))
		CreateObject(FBMP, Random(LandscapeWidth()), 0, -1);
...
//Spawnschutz 
global func FxSpawnTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
	...
	if (iEffectTime > 150)
	{
		/*AddEffect("Normal",pTarget,20,1,0,0,EffectVar(0,pTarget,iEffectNumber));*/
