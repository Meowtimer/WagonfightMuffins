...
protected func CalcValue()
{
	var iSpell;
	if (idSpell)
		iSpell = GetValue(0, idSpell);
	return GetValue(0, GetID()) + iSpell * 2 / 3;
}

protected func RejectEntrance()
{
	if (GetAction() == "WannaRemove")
		return 1;
}
...
{
	// Nicht bereit
	if (!(pByClonk->~ReadyToMagic()))
		return 0;
	if (GetAction(pByClonk) != "Walk")
		return 0;
	// Clonk soll anhalten
...
public func SetSpell(id idMagic)
{
	...
	// Aktion f�r die Schriftrolle setzen
	SetAction("Be");
...
private func DoMagic(object pByClonk)
{
	...
	{
		Exit();
		SetAction("WannaRemove", pByClonk);
	}
	else
...
{
	// manche Zauber h�ngen gerne Effekte an den Caster
	if (!GetEffectCount(0, this) && !pAimingClonk)
		RemoveObject();
}
